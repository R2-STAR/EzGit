...
async def semantic_search(body: SearchRequest, db: AsyncSession = Depends(get_db)):
    ...
    if not status or status["status"] != "done":
        raise HTTPException(status_code=400, detail=f"Repo {repo} is not indexed yet. Call POST /search/index first.")
    try:
        query_embedding = await embed_query(body.query)
    except Exception as e:
        raise HTTPException(status_code=500, detail=f"Embedding failed: {str(e)}")
    results = await similarity_search(db, repo, query_embedding, top_k=body.top_k)
    return SearchResponse(query=body.query, repo=repo, results=results, total=len(results))
...
async def index_repo(body: IndexRequest):
    ...
    if status and status["status"] == "indexing":
        return IndexResponse(repo=repo, status="indexing", message="Indexing already in progress.")
    index_repo_task.delay(repo)
    return IndexResponse(repo=repo, status="started", message="Indexing started. Poll GET /search/index-status/{repo}.")
